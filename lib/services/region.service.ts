const API_KEY = process.env.NEXT_PUBLIC_REGION_API_KEY;
...
export const fetchRegions = async (): Promise<string[]> => {
  try {
    ...
    return data.regcodes.map((item: any) => item.name);
  } catch (error) {
 ...
 */
export const fetchCities = async (regionName: string): Promise<string[]> => {
 ...
 * @returns {Promise<RegionCityMap>}
 */
export const getAllRegionCityMap = async (): Promise<RegionCityMap> => {
  try {
