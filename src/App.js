        ...
        const unsubscribe = onAuthStateChanged(auth, (user) => {
            if (user) {
                ...
                const userUnsubscribe = onSnapshot(userRef, (userSnap) => {
                    ...
                    setCurrentUser({ ...user, isFirestoreDataLoaded: true }); // 오류 발생 시에도 기본 정보로 설정
                    setLoading(false);
                });
...
const PwaInstallModal = ({ isOpen, onClose }) => {
    ...
    const ShareIosIcon = () => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="inline-block w-5 h-5 mx-1 align-middle"><path d="M12 22V8"/><path d="m7 13 5-5 5 5"/><path d="M20 13v5a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-5"/></svg>);
    const PlusSquareIcon = () => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="inline-block w-5 h-5 mx-1 align-middle"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M8 12h8"/><path d="M12 8v8"/></svg>);
    const MoreVerticalIcon = () => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="inline-block w-5 h-5 mx-1 align-middle"><circle cx="12" cy="12" r="1"/><circle cx="12" cy="5" r="1"/><circle cx="12" cy="19" r="1"/></svg>);

    const iosInstructions = (<div className="text-center"><p className="text-lg font-semibold mb-4">iOS에서는</p><p className="mb-4">공유 버튼 <ShareIosIcon /> 을 누른 후</p><p className="mb-6">'홈 화면에 추가' <PlusSquareIcon /> 를 선택해주세요.</p></div>);
    const androidInstructions = (<div className="text-center"><p className="text-lg font-semibold mb-4">Android에서는</p><p className="mb-4">오른쪽 상단 메뉴 <MoreVerticalIcon /> 를 누른 후</p><p className="mb-6">'홈 화면에 추가' 또는 '앱 설치'를 선택해주세요.</p></div>);
...
const RegionSetupPage = () => {
    ...
    const [selectedRegion, setSelectedRegion] = useState(''); const [selectedCity, setSelectedCity] = useState('');
    const [loading, setLoading] = useState(false); const [apiLoading, setApiLoading] = useState(true); const [error, setError] = useState(''); const [isSaving, setIsSaving] = useState(false);

    useEffect(() => { if (isSaving && currentUser?.city) { navigate('/home'); } }, [currentUser, isSaving, navigate]);
    useEffect(() => { fetchRegions().then(data => { setRegions(data); setApiLoading(false); }); }, []);
    useEffect(() => { if (selectedRegion) { setApiLoading(true); setCities([]); setSelectedCity(''); fetchCities(selectedRegion).then(data => { setCities(data); if (data.length === 1) setSelectedCity(data[0]); setApiLoading(false); }); } else { setCities([]); } }, [selectedRegion]);
    
    const handleSaveRegion = async () => {
        if (!selectedRegion || !selectedCity) { setError('거주 지역을 모두 선택해주세요.'); return; }
        setLoading(true); setError(''); setIsSaving(true);
        try {
            await setDoc(doc(db, "users", currentUser.uid), { displayName: currentUser.displayName, email: currentUser.email, photoURL: currentUser.photoURL, region: selectedRegion, city: selectedCity, town: '', createdAt: Timestamp.now() }, { merge: true });
        } catch (e) { console.error("Region save error:", e); setError("저장에 실패했습니다. 다시 시도해주세요."); setLoading(false); setIsSaving(false); }
    };
    
    return (<div className="flex flex-col items-center justify-center h-screen bg-white p-4"><div className="text-center mb-8"><h1 className="text-2xl font-bold text-gray-800">환영합니다!</h1><p className="text-gray-600 mt-2">서비스 이용을 위해<br />거주 지역을 설정해주세요.</p></div>{apiLoading && !regions.length ? <LoadingSpinner/> : (<div className="w-full max-w-xs space-y-4"><select value={selectedRegion} onChange={(e) => setSelectedRegion(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00462A]"><option value="">시/도 선택</option>{regions.map(r => <option key={r} value={r}>{r}</option>)}</select><select value={selectedCity} onChange={(e) => setSelectedCity(e.target.value)} disabled={!selectedRegion || apiLoading || cities.length <= 1} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00462A] disabled:bg-gray-200"><option value="">시/군 선택</option>{apiLoading && selectedRegion ? <option>불러오는 중...</option> : cities.map(c => <option key={c} value={c}>{c}</option>)}</select>{error && <p className="text-red-500 text-sm text-center">{error}</p>}<button onClick={handleSaveRegion} disabled={loading || !selectedCity} className="w-full mt-4 bg-[#00462A] text-white font-bold py-3 px-4 rounded-lg hover:bg-[#003a22] transition-colors shadow-lg disabled:bg-gray-400">{loading ? "저장 중..." : "마을N 시작하기"}</button></div>)}</div>);
};

const HomePage = () => {
    const { currentUser, adminSelectedCity } = useAuth(); const navigate = useNavigate();
    const [posts, setPosts] = useState(null); const [buanNews, setBuanNews] = useState(null);
    const [followingPosts, setFollowingPosts] = useState([]); const [userEvents, setUserEvents] = useState({});
    const [likedNews, setLikedNews] = useState(currentUser?.likedNews || []);
    const [detailModalOpen, setDetailModalOpen] = useState(false); const [selectedNews, setSelectedNews] = useState(null);
    ...
    useEffect(() => {
        ...
        const createSubscription = (coll, baseQuery, setData) => onSnapshot(query(collection(db, coll), ...baseQuery, orderBy("createdAt", "desc")), (s) => setData(s.docs.map(d => ({ id: d.id, ...d.data() }))), () => setData([]));
        
        const unsubscribes = [
            createSubscription('posts', currentTargetCity ? [where("city", "==", currentTargetCity)] : [], setPosts),
            createSubscription('news', currentTargetCity ? [where("city", "==", currentTargetCity)] : [], setBuanNews),
            onSnapshot(query(collection(db, `users/${currentUser.uid}/events`)), (snapshot) => { const ev = {}; snapshot.forEach(d => { const e = { id: d.id, ...d.data()}; if (!ev[e.date]) ev[e.date] = []; ev[e.date].push(e); }); setUserEvents(ev); })
        ];

        if (currentUser.following?.length > 0) { unsubscribes.push(onSnapshot(query(collection(db, "posts"), where('authorId', 'in', currentUser.following.slice(0, 10)), orderBy("createdAt", "desc"), limit(10)), s => setFollowingPosts(s.docs.map(d => ({ id: d.id, ...d.data() }))))); }
    ...
    const handleLikeNews = async (newsItem) => {
        if (!currentUser || !newsItem) return; const userRef = doc(db, 'users', currentUser.uid); const isLiked = likedNews.includes(newsItem.id);
        try { await updateDoc(userRef, { likedNews: isLiked ? arrayRemove(newsItem.id) : arrayUnion(newsItem.id) });
            if (newsItem.date) { const eventsRef = collection(db, `users/${currentUser.uid}/events`); const q = query(eventsRef, where("newsId", "==", newsItem.id), limit(1));
                if (!isLiked && (await getDocs(q)).empty) await addDoc(eventsRef, { title: newsItem.title, date: newsItem.date, type: 'news', newsId: newsItem.id });
                else if (isLiked) { const batch = writeBatch(db); (await getDocs(q)).forEach(d => batch.delete(d.ref)); await batch.commit(); }
            }
        } catch (error) { console.error("Like/Calendar Error:", error); alert("작업 처리 중 오류 발생"); }
    };
    
    const handleDeleteNews = async (newsId, imagePath) => {
      if (!currentUser.isAdmin || !window.confirm("정말로 이 소식을 삭제하시겠습니까?")) return;
      try { if (imagePath) await deleteObject(ref(storage, imagePath)); await deleteDoc(doc(db, 'news', newsId)); alert("소식이 삭제되었습니다."); }
    ...
    return (<div className="p-4 space-y-8"><Modal isOpen={detailModalOpen} onClose={() => setDetailModalOpen(false)}>{selectedNews && ( <div><h2 className="text-2xl font-bold mb-4">{selectedNews.title}</h2> <p className="text-gray-700 whitespace-pre-wrap">{selectedNews.content}</p></div> )}</Modal><section><div className="flex justify-between items-center mb-3"> <h2 className="text-lg font-bold">지금 {displayCity}에서는</h2> <Link to="/news" className="text-sm font-medium text-gray-500 hover:text-gray-800">더 보기 <ChevronRight className="inline-block" size={14} /></Link> </div><div className="flex overflow-x-auto gap-4 pb-3" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>{buanNews.length > 0 ? ( buanNews.map((news) => ( <div key={news.id} className="w-4/5 md:w-3/5 flex-shrink-0"> <NewsCard {...{ news, isAdmin: currentUser.isAdmin, openDetailModal, handleDeleteNews, handleLikeNews, isLiked: likedNews.includes(news.id) }} /> </div>)) ) : ( <div className="text-center text-gray-500 w-full p-8 bg-gray-100 rounded-lg">아직 등록된 소식이 없습니다.</div> )}</div></section><section><div className="flex justify-between items-center mb-3"> <h2 className="text-lg font-bold">{displayCity} 달력</h2> <Link to="/calendar" className="text-sm font-medium text-gray-500 hover:text-gray-800">자세히 <ChevronRight className="inline-block" size={14} /></Link> </div><Calendar events={userEvents} onDateClick={(date) => navigate('/calendar', { state: { date } })} /></section><section><div className="flex justify-between items-center mb-3"> <h2 className="text-lg font-bold">지금 인기있는 글</h2> <Link to="/board" className="text-sm font-medium text-gray-500 hover:text-gray-800">더 보기 <ChevronRight className="inline-block" size={14} /></Link> </div><div className="space-y-3">{popularPosts.length > 0 ? (popularPosts.map(post => { const style = getCategoryStyle(post.category, post.city); return ( <div key={post.id} onClick={() => navigate(`/post/${post.id}`)} className="bg-white p-3 rounded-xl shadow-sm border border-gray-200 flex items-center gap-3 cursor-pointer"> <span className={`text-xs font-bold ${style.text} ${style.bg} px-2 py-1 rounded-md`}>{post.category}</span> <p className="truncate flex-1">{post.title}</p> <div className="flex items-center text-xs text-gray-400 gap-2"> <Heart size={14} className="text-red-400" /> <span>{post.likes?.length || 0}</span> </div> </div> ); })) : (<p className="text-center text-gray-500 py-4">아직 인기글이 없어요.</p>)}</div></section><section><div className="flex justify-between items-center mb-3"> <h2 className="text-lg font-bold">팔로잉</h2> </div><div className="space-y-3">{followingPosts.length > 0 ? (followingPosts.map(post => { const style = getCategoryStyle(post.category, post.city); return ( <div key={post.id} onClick={() => navigate(`/post/${post.id}`)} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 cursor-pointer"><div className="flex items-center gap-2 mb-2"> <span className={`text-xs font-bold ${style.text} ${style.bg} px-2 py-1 rounded-md`}>{post.category}</span> <h3 className="font-bold text-md truncate flex-1">{post.title}</h3> </div><p className="text-gray-600 text-sm mb-3 truncate">{post.content}</p><div className="flex justify-between items-center text-xs text-gray-500"> <div><span onClick={(e) => { e.stopPropagation(); navigate(`/profile/${post.authorId}`); }} className="font-semibold cursor-pointer hover:underline">{post.authorName}</span><span className="mx-1">·</span><span>{timeSince(post.createdAt)}</span> </div> <div className="flex items-center gap-3"><div className="flex items-center gap-1"><Heart size={14} className={post.likes?.includes(currentUser.uid) ? 'text-red-500 fill-current' : 'text-gray-400'} /><span>{post.likes?.length || 0}</span></div> <div className="flex items-center gap-1"><MessageCircle size={14} className="text-gray-400" /><span>{post.commentCount || 0}</span></div></div></div></div>); })) : (<p className="text-center text-gray-500 py-4">팔로우하는 사용자의 글이 없습니다.</p>)}</div></section></div>);
};

const NewsPage = () => {
    const { currentUser, adminSelectedCity } = useAuth(); const navigate = useNavigate();
    const [buanNews, setBuanNews] = useState(null); const [likedNews, setLikedNews] = useState(currentUser?.likedNews || []);
    const [detailModalOpen, setDetailModalOpen] = useState(false); const [selectedNews, setSelectedNews] = useState(null); const [activeTag, setActiveTag] = useState('전체');
    ...
    useEffect(() => {
        ...
        if (!currentUser.isAdmin && !targetCity) { setBuanNews([]); return; }
        const baseQuery = targetCity ? [where("city", "==", targetCity)] : [];
        const q = query(collection(db, "news"), ...baseQuery, orderBy("createdAt", "desc"));
        const unsub = onSnapshot(q, (s) => setBuanNews(s.docs.map(d => ({ id: d.id, ...d.data() }))), (e) => { console.error("Error fetching news:", e); setBuanNews([]); });
        return () => unsub();
    }, [currentUser, adminSelectedCity]);
    ...
    return (<div className="p-4">{currentUser.isAdmin && (<button onClick={() => navigate('/news/write')} className="w-full mb-4 bg-[#00462A] text-white font-bold py-3 px-4 rounded-lg hover:bg-[#003a22] shadow-lg flex items-center justify-center gap-2"><PlusCircle size={20} /> 소식 글쓰기</button>)}<div className="flex space-x-2 mb-4 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>{tags.map(tag => (<button key={tag} onClick={() => setActiveTag(tag)} className={`px-4 py-1.5 text-sm font-semibold rounded-full whitespace-nowrap transition-colors ${ activeTag === tag ? 'bg-[#00462A] text-white' : 'bg-gray-200 text-gray-700'}`}>{tag}</button>))}</div><Modal isOpen={detailModalOpen} onClose={() => setDetailModalOpen(false)}>{selectedNews && (<div><h2 className="text-2xl font-bold mb-4">{selectedNews.title}</h2><p className="text-gray-700 whitespace-pre-wrap">{selectedNews.content}</p></div>)}</Modal><div className="space-y-4">{filteredNews.length > 0 ? (filteredNews.map((news) => (<NewsCard key={news.id} {...{news, isAdmin: currentUser.isAdmin, openDetailModal, handleDeleteNews, handleLikeNews, isLiked: likedNews.includes(news.id)}} />))) : (<div className="text-center text-gray-500 py-10 p-8 bg-gray-100 rounded-lg">{activeTag === '전체' ? '등록된 소식이 없습니다.' : `선택한 태그에 해당하는 소식이 없습니다.`}</div>)}</div></div>);
};

const NewsWritePage = () => {
    const navigate = useNavigate(); const location = useLocation(); const itemToEdit = location.state?.itemToEdit; const { currentUser } = useAuth();
    const [title, setTitle] = useState(itemToEdit?.title || ''); const [content, setContent] = useState(itemToEdit?.content || '');
    const [tags, setTags] = useState(itemToEdit?.tags?.join(', ') || ''); const [applyUrl, setApplyUrl] = useState(itemToEdit?.applyUrl || '');
    const [date, setDate] = useState(itemToEdit?.date || ''); const [imageFile, setImageFile] = useState(null);
    const [imagePreview, setImagePreview] = useState(itemToEdit?.imageUrl || null); const [isSubmitting, setIsSubmitting] = useState(false);
    
    const [allRegions, setAllRegions] = useState([]); const [availableCities, setAvailableCities] = useState([]);
    const [selectedPostRegion, setSelectedPostRegion] = useState(itemToEdit?.region || ''); const [selectedPostCity, setSelectedPostCity] = useState(itemToEdit?.city || '');

    useEffect(() => {
        if (currentUser?.isAdmin) {
            fetchRegions().then(sidos => {
                setAllRegions(sidos); if (itemToEdit?.region) fetchCities(itemToEdit.region).then(setAvailableCities);
            });
        }
    }, [currentUser?.isAdmin, itemToEdit]);

    useEffect(() => {
        if (selectedPostRegion) { fetchCities(selectedPostRegion).then(setAvailableCities); if (itemToEdit?.region !== selectedPostRegion) setSelectedPostCity(''); }
    ...
    const handleSubmit = async () => {
        ...
        if (currentUser.isAdmin && (!selectedPostRegion || !selectedPostCity)) { alert('소식을 등록할 지역(시/도, 시/군)을 모두 선택해주세요.'); return; }
        if (isSubmitting) return; setIsSubmitting(true);
        try {
            let imageUrl = itemToEdit?.imageUrl || null, imagePath = itemToEdit?.imagePath || null;
            if (imageFile) {
                if (itemToEdit?.imagePath) await deleteObject(ref(storage, itemToEdit.imagePath)).catch(console.error);
                const newImagePath = `news_images/${Date.now()}_${imageFile.name}`;
                const storageRef = ref(storage, newImagePath); await uploadBytes(storageRef, imageFile);
                imageUrl = await getDownloadURL(storageRef); imagePath = newImagePath;
            }
...
const BoardPage = () => {
    const { currentUser, adminSelectedCity } = useAuth(); const navigate = useNavigate();
    const [posts, setPosts] = useState(null); const [filter, setFilter] = useState('전체');
...
const WritePage = () => {
    ...
    const handleImageChange = (e) => { if (e.target.files[0]) { const file = e.target.files[0]; if (imagePreview && imagePreview.startsWith('blob:')) URL.revokeObjectURL(imagePreview); setImageFile(file); setImagePreview(URL.createObjectURL(file)); } };

    const handleSubmit = async () => {
        if (!title.trim() || !content.trim()) { alert('제목과 내용을 모두 입력해주세요.'); return; }
        if (currentUser.isAdmin && (!selectedPostRegion || !selectedPostCity)) { alert('게시글을 등록할 지역을 모두 선택해주세요.'); return; }
        if (isSubmitting) return; setIsSubmitting(true);
        try {
            let imageUrl = itemToEdit?.imageUrl || null, imagePath = itemToEdit?.imagePath || null;
            if (imageFile) {
                if (itemToEdit?.imagePath) await deleteObject(ref(storage, itemToEdit.imagePath)).catch(console.error);
                const newImagePath = `posts/${currentUser.uid}/${Date.now()}_${imageFile.name}`; const storageRef = ref(storage, newImagePath);
                await uploadBytes(storageRef, imageFile); imageUrl = await getDownloadURL(storageRef); imagePath = newImagePath;
            }

            const [region, city] = currentUser.isAdmin ? [selectedPostRegion, selectedPostCity] : [currentUser.region, currentUser.city];
            
            const postData = { title, content, category, imageUrl, imagePath, updatedAt: Timestamp.now(), region, city };
            if (!itemToEdit) { Object.assign(postData, { authorId: currentUser.uid, authorName: currentUser.displayName, createdAt: Timestamp.now(), likes: [], bookmarks: [], commentCount: 0 }); }
            if (itemToEdit) { await updateDoc(doc(db, 'posts', itemToEdit.id), postData); }
            else { await addDoc(collection(db, 'posts'), postData); }
            navigate('/board');
        } catch (error) { alert(`오류가 발생했습니다: ${error.message}`); } finally { setIsSubmitting(false); }
    };
...
const UserProfilePage = () => {
    const { userId } = useParams(); const navigate = useNavigate(); const { currentUser, adminSelectedCity, setAdminSelectedCity } = useAuth();
    const [profileUser, setProfileUser] = useState(null); const [userPosts, setUserPosts] = useState(null);
    const [allRegions, setAllRegions] = useState([]); const [availableCities, setAvailableCities] = useState([]);
    const [selectedAdminRegion, setSelectedAdminRegion] = useState(''); const [selectedAdminCity, setSelectedAdminCity] = useState(adminSelectedCity || '');
    const [isAdminModeOn, setIsAdminModeOn] = useState(!!adminSelectedCity || false);

    useEffect(() => { if (currentUser?.isAdmin) fetchRegions().then(setAllRegions); }, [currentUser?.isAdmin]);
    useEffect(() => { if (selectedAdminRegion && selectedAdminRegion !== "전체") fetchCities(selectedAdminRegion).then(setAvailableCities); else setAvailableCities([]); }, [selectedAdminRegion]);
    useEffect(() => { if (!userId) return; const unsubUser = onSnapshot(doc(db, 'users', userId), d => setProfileUser(d.exists() ? { id: d.id, uid: d.id, ...d.data() } : null)); const unsubPosts = onSnapshot(query(collection(db, 'posts'), where("authorId", "==", userId), orderBy("createdAt", "desc")), s => setUserPosts(s.docs.map(d => ({id: d.id, ...d.data()})))); return () => { unsubUser(); unsubPosts(); }; }, [userId]);

    const handleAdminModeToggle = () => { setIsAdminModeOn(!isAdminModeOn); if (isAdminModeOn) { setAdminSelectedCity(null); setSelectedAdminRegion(''); setSelectedAdminCity(''); } };
    const handleRegionViewChange = () => { const finalCity = selectedAdminRegion === "전체" ? null : selectedAdminCity; setAdminSelectedCity(finalCity); alert(`${finalCity ? `${selectedAdminRegion} ${finalCity}` : '전체 보기'} (으)로 뷰가 변경되었습니다.`); navigate('/home'); };
    const handleFollow = async () => { if (!currentUser || !profileUser) return; try { const isFollowing = profileUser.followers?.includes(currentUser.uid); const batch = writeBatch(db); batch.update(doc(db, 'users', currentUser.uid), { following: isFollowing ? arrayRemove(userId) : arrayUnion(userId) }); batch.update(doc(db, 'users', userId), { followers: isFollowing ? arrayRemove(currentUser.uid) : arrayUnion(currentUser.uid) }); await batch.commit(); } catch(e) { console.error("Follow error:", e); alert("팔로우 처리 중 오류 발생"); }};
    const handleLogout = async () => { if (window.confirm('로그아웃 하시겠습니까?')) { await signOut(auth); navigate('/start'); } };
    const handleMessage = () => navigate(`/chat/${[currentUser.uid, userId].sort().join('_')}`, { state: { recipientId: userId, recipientName: profileUser.displayName }});
    
    if(!profileUser || !userPosts) return <LoadingSpinner />;
    
    const isMyProfile = currentUser.uid === userId; const isFollowing = profileUser.followers?.includes(currentUser.uid) || false;
    ...
    return (<div className="p-4 pb-16"><div className="flex items-center mb-6"><div className="w-16 h-16 rounded-full mr-4 flex-shrink-0 bg-gray-200 overflow-hidden flex items-center justify-center">{profileUser.photoURL ? (<img src={profileUser.photoURL} alt={profileUser.displayName} className="w-full h-full object-cover" />) : (<UserCircle size={64} className="text-gray-400" />)}</div><div className="flex-1"><h2 className="text-xl font-bold">{profileUser.displayName}</h2><p className="text-sm text-gray-600 mt-1">{profileUser.bio || '자기소개를 입력해주세요.'}</p><p className="text-xs text-gray-500 mt-1">{userLocation}</p><div className="text-sm text-gray-500 mt-2"><span>팔로워 {profileUser.followers?.length || 0}</span><span className="mx-2">·</span><span>팔로잉 {profileUser.following?.length || 0}</span></div></div></div><div className="flex gap-2 mb-6">{isMyProfile ? (<><button onClick={() => navigate('/profile/edit')} className="flex-1 p-2 text-sm font-semibold rounded-lg bg-gray-200 text-gray-800 flex items-center justify-center gap-1"><Edit size={16} /> 프로필 편집</button><button onClick={handleLogout} className="flex-1 p-2 text-sm font-semibold rounded-lg bg-gray-200 text-gray-800 flex items-center justify-center gap-1"><LogOut size={16} /> 로그아웃</button></>) : (<><button onClick={handleFollow} className={`flex-1 px-4 py-2 text-sm font-semibold rounded-lg flex items-center justify-center gap-1.5 ${ isFollowing ? 'bg-gray-200 text-[#00462A]' : 'bg-[#00462A] text-white' }`}>{isFollowing ? '✓ 팔로잉' : '+ 팔로우'}</button><button onClick={handleMessage} className="flex-1 px-4 py-2 text-sm font-semibold rounded-lg bg-white text-[#00462A] border border-[#00462A] flex items-center justify-center gap-1.5"><MessageSquare size={16} /> 메시지</button></>)}</div>{isMyProfile && currentUser.isAdmin && (<div className="mb-6 p-4 bg-gray-100 rounded-lg space-y-4"><h3 className="text-md font-bold text-gray-800">관리자 도구</h3><div className="flex items-center justify-between"><label htmlFor="admin-mode-toggle" className="font-semibold text-gray-700">관리자 모드</label><button onClick={handleAdminModeToggle} id="admin-mode-toggle" className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors ${isAdminModeOn ? 'bg-green-600' : 'bg-gray-300'}`}><span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform ${isAdminModeOn ? 'translate-x-6' : 'translate-x-1'}`}/></button></div>{isAdminModeOn && (<div className="space-y-3"><div><label htmlFor="region-select" className="block text-sm font-medium text-gray-600 mb-1">시/도 선택</label><select id="region-select" value={selectedAdminRegion} onChange={(e) => { setSelectedAdminRegion(e.target.value); setSelectedAdminCity(''); }} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00462A]"><option value="">시/도 선택...</option><option value="전체">전체 보기 (관리자)</option>{allRegions.map(r => <option key={r} value={r}>{r}</option>)}</select></div>{selectedAdminRegion && selectedAdminRegion !== '전체' && (<div><label htmlFor="city-select" className="block text-sm font-medium text-gray-600 mb-1">시/군 선택</label><select id="city-select" value={selectedAdminCity} onChange={(e) => setSelectedAdminCity(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00462A]" disabled={!availableCities.length}><option value="">시/군 선택...</option>{availableCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>)}<button onClick={handleRegionViewChange} disabled={!selectedAdminRegion || (selectedAdminRegion !== '전체' && !selectedAdminCity)} className="w-full mt-2 bg-[#00462A] text-white font-bold py-2 px-4 rounded-lg hover:bg-[#003a22] disabled:bg-gray-400">뷰 변경 적용</button></div>)}</div>)}<div className="space-y-3"><h3 className="text-lg font-bold">작성한 글</h3>{userPosts.length > 0 ? userPosts.map(post => ( <div key={post.id} onClick={() => navigate(`/post/${post.id}`)} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 cursor-pointer"><h4 className="font-bold text-md truncate mb-1">{post.title}</h4><p className="text-gray-600 text-sm truncate">{post.content}</p></div>)) : ( <p className="text-center text-gray-500 py-10">아직 작성한 글이 없습니다.</p> )}</div></div>);
};

const SearchPage = () => {
    const { currentUser, adminSelectedCity } = useAuth(); const navigate = useNavigate();
    const [searchTerm, setSearchTerm] = useState(''); const [results, setResults] = useState([]); const [loading, setLoading] = useState(false);
    const targetCity = adminSelectedCity || (currentUser.isAdmin ? null : currentUser.city); const displayCity = adminSelectedCity || (currentUser.isAdmin ? '전체' : currentUser.city);

    const handleSearch = async () => { if (!searchTerm.trim()) return; setLoading(true); try { const constraints = [ where('title', '>=', searchTerm), where('title', '<=', searchTerm + '\uf8ff') ]; if (targetCity) constraints.unshift(where('city', '==', targetCity)); const q = query(collection(db, 'posts'), ...constraints); setResults((await getDocs(q)).docs.map(d => ({ id: d.id, ...d.data() }))); } catch (e) { console.error("Search error: ", e); alert("검색 중 오류 발생"); } finally { setLoading(false); } };
    return (<div className="p-4"><div className="relative mb-4 flex gap-2"><input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSearch()} placeholder={targetCity ? `${displayCity} 내에서 검색...` : '전체 게시물에서 검색...'} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-[#00462A]" /><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} /><button onClick={handleSearch} className="px-4 py-2 bg-[#00462A] text-white rounded-full font-semibold">검색</button></div>{loading ? <LoadingSpinner /> : (<div className="space-y-3">{results.length > 0 ? results.map(post => (<div key={post.id} onClick={() => navigate(`/post/${post.id}`)} className="bg-white p-4 rounded-xl shadow-sm border-gray-200 cursor-pointer"><h3 className="font-bold text-md truncate mb-1">{post.title}</h3><p className="text-gray-600 text-sm truncate">{post.content}</p></div>)) : (searchTerm && <p className="text-center text-gray-500 py-10">검색 결과가 없습니다.</p>)}</div>)}</div>);
};
...
const ChatListPage = () => {
    const { currentUser } = useAuth(); const navigate = useNavigate(); const [chats, setChats] = useState(null);
    useEffect(() => { if (!currentUser) return; const q = query(collection(db, 'chats'), where('members', 'array-contains', currentUser.uid), orderBy('lastMessage.createdAt', 'desc')); const unsub = onSnapshot(q, async (s) => { const cData = await Promise.all(s.docs.map(async d => { const cd = d.data(); const oId = cd.members.find(id => id !== currentUser.uid); if (!oId) return null; const uDoc = await getDoc(doc(db, 'users', oId)); return { id: d.id, ...cd, otherUser: uDoc.exists() ? {uid: uDoc.id, ...uDoc.data()} : { displayName: '알 수 없음', uid: oId } }; })); setChats(cData.filter(Boolean)); }, (e) => { console.error("Chat list error:", e); setChats([]); }); return unsub; }, [currentUser]);
    
    if (chats === null) return <LoadingSpinner />;
    return (<div className="p-4"><h2 className="text-xl font-bold mb-4">채팅 목록</h2><div className="space-y-3">{chats.length > 0 ? chats.map(chat => (<div key={chat.id} onClick={() => navigate(`/chat/${chat.id}`, { state: { recipientId: chat.otherUser.uid, recipientName: chat.otherUser.displayName }})} className="bg-white p-4 rounded-xl shadow-sm border-gray-200 cursor-pointer flex items-center gap-4"><div className="w-12 h-12 rounded-full bg-gray-300 shrink-0 overflow-hidden flex items-center justify-center">{chat.otherUser.photoURL ? <img src={chat.otherUser.photoURL} alt={chat.otherUser.displayName} className="w-full h-full object-cover" /> : <UserCircle size={48} className="text-gray-400"/>}</div><div className="flex-1 overflow-hidden"><h3 className="font-bold">{chat.otherUser.displayName}</h3><p className="text-sm text-gray-500 truncate">{chat.lastMessage?.text || '메시지를 보내보세요.'}</p></div></div>)) : (<p className="text-center text-gray-500 py-10">진행중인 대화가 없습니다.</p>)}</div></div>);
};

const ChatPage = () => {
    const { currentUser } = useAuth(); const { chatId } = useParams(); const location = useLocation();
    const recipientId = location.state?.recipientId;
    const [messages, setMessages] = useState([]); const [newMessage, setNewMessage] = useState(''); const [isAllowed, setIsAllowed] = useState(true); const [loading, setLoading] = useState(true);
    const messagesEndRef = useRef(null);

    useEffect(() => {
        if (!chatId || !currentUser || !recipientId) { setLoading(false); setIsAllowed(false); return; }
        const chatRef = doc(db, 'chats', chatId); const messagesRef = collection(chatRef, 'messages'); const q = query(messagesRef, orderBy('createdAt', 'asc'));
        const messagesUnsub = onSnapshot(q, s => setMessages(s.docs.map(d => ({ id: d.id, ...d.data() }))), () => setLoading(false));
        const chatUnsub = onSnapshot(chatRef, chatSnap => { if (chatSnap.exists() && !chatSnap.data().members?.includes(currentUser.uid)) setIsAllowed(false); setLoading(false); });
        return () => { messagesUnsub(); chatUnsub(); };
    }, [chatId, currentUser, recipientId]);
    
    useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages]);

    const handleSendMessage = async (e) => { e.preventDefault(); if (!newMessage.trim() || !recipientId) return; const msg = { text: newMessage, senderId: currentUser.uid, createdAt: Timestamp.now() }; const chatRef = doc(db, 'chats', chatId); try { const batch = writeBatch(db); if (!(await getDoc(chatRef)).exists()) batch.set(chatRef, { members: [currentUser.uid, recipientId], lastMessage: msg }); else batch.update(chatRef, { lastMessage: msg }); batch.set(doc(collection(chatRef, 'messages')), msg); await batch.commit(); setNewMessage(''); } catch (error) { console.error("Send message error:", error); alert("메시지 전송 실패"); } };
    
    if (loading) return <LoadingSpinner />; if (!isAllowed) return <div className="p-4 text-center text-red-500">채팅방 접근 권한이 없습니다.</div>;

    return (<div className="flex flex-col h-screen"><Header /><div className="flex-1 overflow-y-auto p-4 space-y-4">{messages.map(msg => (<div key={msg.id} className={`flex ${msg.senderId === currentUser.uid ? 'justify-end' : 'justify-start'}`}><div className={`max-w-xs p-3 rounded-lg ${msg.senderId === currentUser.uid ? 'bg-green-500 text-white' : 'bg-gray-200'}`}><p>{msg.text}</p></div></div>))}<div ref={messagesEndRef} /></div><div className="bg-white border-t p-3" style={{paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom))'}}><form onSubmit={handleSendMessage} className="relative flex items-center"><input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="메시지 입력" className="w-full pl-4 pr-12 py-2 bg-gray-100 rounded-full focus:outline-none focus:ring-2 focus:ring-[#00462A]" /><button type="submit" className="absolute right-2 p-2 rounded-full text-gray-500 hover:bg-gray-200"><Send size={20} /></button></form></div></div>);
};

const ClubListPage = () => {
    const { currentUser } = useAuth(); const navigate = useNavigate();
    const [clubs, setClubs] = useState(null); const [passwordModalOpen, setPasswordModalOpen] = useState(false);
    const [selectedClub, setSelectedClub] = useState(null); const [password, setPassword] = useState(''); const [activeTab, setActiveTab] = useState('내 모임');
    
    useEffect(() => { const unsub = onSnapshot(query(collection(db, "clubs"), orderBy("createdAt", "desc")), (s) => setClubs(s.docs.map(d => ({ id: d.id, ...d.data() })))); return unsub; }, []);
    if (clubs === null) return <LoadingSpinner />;

    const displayedClubs = activeTab === '내 모임' ? clubs.filter(c => c.members?.includes(currentUser.uid)) : clubs;
    const handleEnterClub = (club) => { if (club.members?.includes(currentUser.uid) || !club.password) navigate(`/clubs/${club.id}`); else { setSelectedClub(club); setPasswordModalOpen(true); } };
    const handlePasswordSubmit = async () => { if (!selectedClub || !password) return; if (password === selectedClub.password) { try { await updateDoc(doc(db, 'clubs', selectedClub.id), { members: arrayUnion(currentUser.uid) }); navigate(`/clubs/${selectedClub.id}`); } catch (e) { alert("모임 입장 실패"); } finally { setPasswordModalOpen(false); setPassword(''); setSelectedClub(null); } } else alert('비밀번호가 일치하지 않습니다.'); };
    return (<div className="p-4"><Modal isOpen={passwordModalOpen} onClose={() => { setPasswordModalOpen(false); setPassword(''); }}><h3 className="text-lg font-bold mb-4">{selectedClub?.name}</h3><p className="mb-4">비밀번호가 필요한 모임입니다.</p><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handlePasswordSubmit()} placeholder="비밀번호" className="w-full p-2 border rounded mb-4" /><button onClick={handlePasswordSubmit} className="w-full bg-[#00462A] text-white py-2 rounded">입장하기</button></Modal><div className="flex justify-between items-center mb-4"><div className="flex bg-gray-200 rounded-lg p-1"><button onClick={() => setActiveTab('내 모임')} className={`px-4 py-1 text-sm font-semibold rounded-md ${activeTab === '내 모임' ? 'bg-white shadow' : 'text-gray-600'}`}>내 모임</button><button onClick={() => setActiveTab('전체 모임')} className={`px-4 py-1 text-sm font-semibold rounded-md ${activeTab === '전체 모임' ? 'bg-white shadow' : 'text-gray-600'}`}>전체 모임</button></div><button onClick={() => navigate('/clubs/create')} className="bg-[#00462A] text-white font-bold py-2 px-3 rounded-lg hover:bg-[#003a22] flex items-center gap-1 text-sm"><PlusCircle size={16} /> 만들기</button></div><div className="space-y-3">{displayedClubs.length > 0 ? displayedClubs.map(club => (<div key={club.id} onClick={() => handleEnterClub(club)} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 cursor-pointer flex items-center gap-4"><img src={club.photoURL} alt={club.name} className="w-16 h-16 rounded-lg object-cover bg-gray-200 shrink-0" /><div className="flex-1 overflow-hidden"><h3 className="font-bold text-lg">{club.name}</h3><p className="text-sm text-gray-500 truncate">{club.description}</p><div className="text-xs text-gray-400 mt-1 flex items-center">{club.password && <Lock size={12} className="mr-1" />}<span>멤버 {club.members?.length || 1}명</span><span className="mx-1">·</span><span onClick={(e) => { e.stopPropagation(); navigate(`/profile/${club.creatorId}`); }} className="hover:underline">{club.creatorName}</span></div></div></div>)) : (<div className="text-center text-gray-500 py-20">{activeTab === '내 모임' ? '가입한 모임이 없습니다.' : '생성된 모임이 없습니다.'}</div>)}</div></div>);
};

const ClubCreatePage = () => {
    const navigate = useNavigate(); const { currentUser } = useAuth();
    const [name, setName] = useState(''); const [desc, setDesc] = useState(''); const [category, setCat] = useState(''); const [password, setPassword] = useState('');
    const [imageFile, setImageFile] = useState(null); const [imagePreview, setImagePreview] = useState(null); const [isSubmitting, setIsSubmitting] = useState(false);
    
    const handleSubmit = async () => {
        if (!name || !desc || !imageFile) { alert('모임 이름, 소개, 대표 사진은 필수입니다.'); return; } setIsSubmitting(true);
        try { const imagePath = `club_images/${currentUser.uid}/${Date.now()}_${imageFile.name}`; const storageRef = ref(storage, imagePath); await uploadBytes(storageRef, imageFile); const photoURL = await getDownloadURL(storageRef); await addDoc(collection(db, 'clubs'), { name, description: desc, category, password, photoURL, imagePath, creatorId: currentUser.uid, creatorName: currentUser.displayName, members: [currentUser.uid], createdAt: Timestamp.now(), }); navigate('/clubs'); }
        catch (e) { console.error(e); alert('모임 생성 실패'); } finally { setIsSubmitting(false); }
    };
    
    const handleImageChange = (e) => { const f = e.target.files[0]; if (f) { if (imagePreview && imagePreview.startsWith('blob:')) URL.revokeObjectURL(imagePreview); setImageFile(f); setImagePreview(URL.createObjectURL(f)); }};
...
const ClubDetailPage = () => {
    const { clubId } = useParams(); const navigate = useNavigate(); const { currentUser } = useAuth();
    const [club, setClub] = useState(null); const [members, setMembers] = useState([]); const [posts, setPosts] = useState([]);
    useEffect(() => {
...
const PageLayout = ({ children, hasHeader=true, hasFooter=true }) => (
    <div className="relative flex flex-col min-h-screen">
        {hasHeader && <Header />}
        <main className={`flex-grow bg-white ${hasHeader ? 'pt-16' : ''} ${hasFooter ? 'pb-20' : ''}`}>
            {children}
        </main>
        {hasFooter && <BottomNav />}
    </div>
...
const Header = () => {
    const navigate = useNavigate(); const location = useLocation(); const { currentUser, adminSelectedCity } = useAuth();
    ...
    const getPageTitle = () => {
        ...
        if (pathname === '/home') {
            if (adminSelectedCity) return `마을N ${adminSelectedCity.replace(/(특별시|광역시|특별자치시|도|시|군|구)$/, '')}`;
            if (currentUser.isAdmin) return '마을N';
            return `마을N ${currentUser.city?.replace(/(특별시|광역시|특별자치시|도|시|군|구)$/, '') || ''}`;
        }
        if(pathname.startsWith('/profile/')) return '프로필'; if(pathname.startsWith('/post/')) return '게시글';
        if(pathname.startsWith('/chat/')) return location.state?.recipientName || '채팅'; if(pathname.startsWith('/clubs/')) return '모임';
        return { '/news': '소식', '/board': '게시판', '/calendar': '달력', '/search': '검색', '/notifications': '알림', '/chats': '채팅' }[pathname] || '마을N';
    };
    
    return (<header className="fixed top-0 left-0 right-0 max-w-sm mx-auto bg-white/80 backdrop-blur-sm z-30 px-4 py-3 flex justify-between items-center border-b border-gray-200 h-16"><div className="flex items-center gap-2 flex-1">{!isMainPage ? (<button onClick={() => navigate(-1)} className="p-1 -ml-2"><ArrowLeft size={24} /></button>) : (<Logo size={28} />)}<h1 className="text-xl font-bold text-gray-800 truncate">{getPageTitle()}</h1></div><div className="flex items-center gap-3"><Link to="/search" className="p-1"><Search size={24} className="text-gray-600" /></Link><Link to="/chats" className="p-1"><MessageSquare size={24} className="text-gray-600" /></Link><Link to="/notifications" className="p-1"><Bell size={24} className="text-gray-600" /></Link>{currentUser && (<Link to={`/profile/${currentUser.uid}`}>{currentUser.photoURL ? <img src={currentUser.photoURL} alt="profile" className="w-8 h-8 rounded-full bg-gray-200 object-cover"/> : <UserCircle size={32} className="text-gray-400"/>}</Link>)}</div></header>);
};

const BottomNav = () => {
    const location = useLocation();
    const navItems = [ { path: '/home', icon: Home, label: '홈' }, { path: '/board', icon: LayoutGrid, label: '게시판' }, { path: '/news', icon: Newspaper, label: '소식' }, { path: '/clubs', icon: Users, label: '클럽' }, { path: '/benefits', icon: TicketPercent, label: '혜택' } ];
    return (<footer className="fixed bottom-0 left-0 right-0 max-w-sm mx-auto z-20"><div className="bg-white px-3 pt-2 pb-3 border-t border-gray-200 shadow-[0_-1px_4px_rgba(0,0,0,0.05)]" style={{paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom))'}}><div className="flex justify-around items-center">{navItems.map(item => { const isActive = location.pathname.startsWith(item.path); return (<Link to={item.path} key={item.path} onClick={(e) => { if (item.path === '/benefits') { e.preventDefault(); alert('서비스 준비중입니다.'); } }} className="text-center p-2 rounded-lg w-1/5"><item.icon className={`w-6 h-6 mx-auto ${isActive ? 'text-[#00462A]' : 'text-gray-500'}`} /><span className={`text-xs font-medium ${isActive ? 'text-[#00462A] font-bold' : 'text-gray-500'}`}>{item.label}</span></Link>); })}</div></div></footer>);
};

const ProtectedRoute = ({ children }) => {
    const { currentUser, loading } = useAuth();
    const location = useLocation();
    if (loading || (currentUser && !currentUser.isFirestoreDataLoaded)) { return (<div className="max-w-sm mx-auto bg-white min-h-screen flex items-center justify-center"><LoadingSpinner /></div>); }
    if (!currentUser) { return location.pathname === '/start' ? children : <Navigate to="/start" replace />; }
    if (currentUser.isAdmin) { if (location.pathname === '/start' || location.pathname === '/region-setup') return <Navigate to="/home" replace />; return children; }
    if (!currentUser.city) { return location.pathname === '/region-setup' ? children : <Navigate to="/region-setup" replace />; }
    if (currentUser.city) { if (location.pathname === '/start' || location.pathname === '/region-setup') return <Navigate to="/home" replace />; return children; }
                            ...
                            <Route path="/*" element={
                                <ProtectedRoute>
                                    <Routes>
                                        ...
                                        {/* Profile related pages with Header but no Footer */}
                                        ...
                                        {/* Fullscreen pages with no Header/Footer */}
